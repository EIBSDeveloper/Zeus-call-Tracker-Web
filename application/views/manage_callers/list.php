												...
												<div class="tab-pane fade show active" id="kt_tab_users" role="tabpanel">
													<div class="row mt-4">
														...
														<div class="col-lg-8 mb-2">
															<div class="d-flex justify-content-end gap-2 mb-2 mt-2">
																...
																<div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
																	<div class="px-7 py-5" data-kt-user-table-filter="form">
																		<form method="post" action="<?php echo base_url() ?>Manage_callers" id="fill_form">
																			<div class="scroll-y mh-325px">
																				<div class="mb-2">
																					<label class="form-label">Caller</label>
																					<input type="text" class="form-control form-control-lg_1 form-control-solid" placeholder="Enter Caller Name" name="caller_name_fill" value="<?php echo $caller_name_fill ?? "" ; ?>">
																				</div>
																				<input type="hidden" class="sorting_filter_class" name="sorting_filter" id="sorting_filter" value="<?php echo $perpage ? $perpage : 10; ?>" />
																				<div class="mb-2">
																					<label class="form-label">Department</label>
																					<select class="form-select form-select-solid" data-control="select2" name="department_id_fill">
																						<option value="">All</option>
																						<?php if(isset($dept_list)) {?>
																						<?php foreach ($dept_list as $i => $c_list) { ?>
																							<option value="<?php echo  $c_list->department_id   ?>" <?php echo $department_id_fill == $c_list->department_id ? "selected" : '' ?>>
																								<?php echo  $c_list->department_name ?></option>
																						<?php } ?>
																						<?php } ?>
																					</select>
																				...
																				<div class="mb-2">
																					<div class="d-flex justify-content-between">
																						<div class="form-check mt-4">
																							<input class="form-check-input" type="checkbox" name="is_manager_fill" value="1" <?php echo $is_manager_fill==1 ? "checked":"" ;?> />
																							<label class="form-check-label fw-semibold fs-6 text-black" >Manager</label>
																						</div>
																						<div class="form-check mt-4">
																							<input class="form-check-input" value="1" type="checkbox" <?php echo $is_inactive_fill==1 ? "checked":"" ;?> name="is_inactive_fill"/>
																							<label class="form-check-label fw-semibold fs-6 text-black" >In-active Caller</label>
																						</div>
																					</div>
																				</div>
																			...
																			<div class="d-flex justify-content-end">
																				<a href="<?php echo base_url(); ?>Manage_callers" class="btn btn-light btn-active-light-primary fw-semibold me-2 px-6" data-kt-menu-dismiss="true" data-kt-user-table-filter="reset">Reset</a>
													...
													<div class="row">
														...
														<div class="col-lg-12">
															<table class="table align-middle table-hover fs-7 gy-3 gs-2 list_page">
																...
																<?php if(isset($caller_list)){?>
																	<?php foreach ($caller_list as $i => $caller) { ?>
																		...
																		?>
																		<?php 
																		$incoming_call_count= incoming_call_count($caller->user_id) ;
																		$outcoming_call_count= outcoming_call_count($caller->user_id) ;
																		$missed_call_count= missed_call_count($caller->user_id) ;
																		$rejected_call_count= rejected_call_count($caller->user_id) ;

																		$avrg_outgoing = 0;
																		$avrg_missed = 0;

																		if ($incoming_call_count > 0 && $missed_call_count >= 0 && $rejected_call_count >= 0) {
																			$total_calls = $incoming_call_count + $missed_call_count + $rejected_call_count;
																			if ($total_calls > 0) {
																				$avrg_missed = ($missed_call_count / $total_calls) * 100;
																			} else {
																				$avrg_missed = 0;
																			}
																		}
																		if ($incoming_call_count > 0 && $missed_call_count >= 0 && $rejected_call_count >= 0) {
																			$total_calls_all = $incoming_call_count + $missed_call_count + $rejected_call_count+$outcoming_call_count;
																			if ($total_calls > 0) {
																				$avrg_outgoing = ($outcoming_call_count/$total_calls_all) * 100;
																			} else {
																				$avrg_outgoing = 0;
																			}
																		}
																	...
																	<tr style="<?php if ($caller->status==3){ ?>background-color: #ffa5a5 !important;<?php }?>">
																		...
																		<td class="text-center">
																			<label class="fs-7 text-black fw-bold text-center"><?php echo sprintf("%02d", $incoming_call_count); ?></label>
																		</td>
																		<td class="text-center">
																			<label class="fs-7 text-black fw-bold text-center"><?php echo sprintf("%02d", $outcoming_call_count); ?></label>
																			<div class="d-block text-center">
																				<label class="badge badge-success fs-8 text-white me-1"><?php echo number_format($avrg_outgoing); ?> %</label>
																			</div>
																		</td>
																		<td class="text-center">
																			<label class="fs-7 text-black fw-bold text-center"><?php echo sprintf("%02d", $missed_call_count); ?></label>
																			<div class="d-block text-center">
																				<label class="badge badge-info fs-8 text-white me-1"><?php echo number_format($avrg_missed); ?> %</label>
																			</div>
																		</td>
																		<td class="text-center">
																			<label class="fs-7 text-black text-center fw-bold"><?php echo sprintf("%02d", $rejected_call_count); ?></label>
																		</td>
																		<td>
															...
															<div class="row">
															<div class="col-lg-3">Showing <?php echo $page; ?> to <?php echo (($page + $perpage_count) > count($caller_list)) ? count($caller_list) : $page + $perpage_count; ?> of <?php echo count($caller_list); ?> entries</div>
																<div class="col-lg-9 d-flex justify-content-end">
																	...
																	<form method="POST" id="filter_form" action="" enctype="multipart/form-data">
																		...
																		<input type="hidden" id="caller_name_fill" name="caller_name_fill" value="<?php echo $caller_name_fill ?? ''; ?>">																		
																		...
																		<input type="hidden" id="department_id_fill" name="department_id_fill" value="<?php echo $department_id_fill ?? ''; ?>">
